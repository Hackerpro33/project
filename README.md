...
```bash
# установка зависимостей, не покидая корень репозитория (использует `npm ci`, если есть lockfile)
./scripts/install_frontend_deps.sh

# если npm сообщит об ошибке ERESOLVE/peerDependencies, удалите `frontend/node_modules`
# и повторно запустите установку; при необходимости добавьте `--legacy-peer-deps`
...
> Бэкенд рассчитан на Python 3.10–3.11 (см. цели форматтеров в `pyproject.toml`).
> При установке на Python 3.13+ не тяните опциональные зависимости `uvicorn[standard]`:
> они подтягивают `uvloop`/`httptools`, которые пока не публикуют стабильные колёса
> под новые версии и могут приводить к сегфолтам. Если нужна поддержка uvloop, ставьте
> его отдельно под совместимую версию Python или задайте `UVICORN_LOOP=asyncio`.
