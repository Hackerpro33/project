...
describe('entities API client', () => {
  ...
  it('lists datasets with pagination defaults', async () => {
  ...
  it('supports dataset list filters', async () => {
    global.fetch.mockResolvedValueOnce({
      ok: true,
      status: 200,
    ...
    expect(global.fetch).toHaveBeenCalledWith(
      '/api/dataset/list?order_by=-created_at&page=3&page_size=50&search=crime&tags=fraud&tags=geo',
      expect.objectContaining({
        headers: expect.objectContaining({ 'Content-Type': 'application/json' }),
      }),
    );
  ...
  it('keeps dataset list on the versioned endpoint when filters are empty', async () => {
    global.fetch.mockResolvedValueOnce({
      ok: true,
      status: 200,
      json: () => Promise.resolve({ items: [] }),
    ...
    await Dataset.list({ search: '', tags: [] });
  ...
  it('propagates errors with response text', async () => {
    ...
    await expect(Dataset.get('missing')).rejects.toThrow('failure');
  });
  ...
  it('lists visualizations with pagination options', async () => {
    global.fetch.mockResolvedValueOnce({
      ok: true,
      status: 200,
      json: () => Promise.resolve({ items: [] }),
    ...
    expect(global.fetch).toHaveBeenCalledWith(
      '/api/visualization/list?order_by=-created_at&page=2&page_size=10&search=map&tags=geo&types=heatmap',
      expect.objectContaining({
        headers: expect.objectContaining({ 'Content-Type': 'application/json' }),
      }),
    );
  ...
  it('keeps visualization list on the versioned endpoint when filters are empty', async () => {
    global.fetch.mockResolvedValueOnce({
      ok: true,
      status: 200,
      json: () => Promise.resolve({ items: [] }),
    ...
    await Visualization.list({ tags: [], types: [] });
  ...
  it('exposes convenience getters that unwrap payloads', async () => {
    global.fetch
      .mockResolvedValueOnce({
        ok: true,
        status: 200,
        json: () => Promise.resolve({ items: [{ id: 'd1' }] }),
      })
      .mockResolvedValueOnce({
        ok: true,
        status: 200,
    ...
    expect(datasets).toEqual([{ id: 'd1' }]);
    expect(visualizations).toEqual([{ id: 'v1' }]);
    expect(global.fetch).toHaveBeenNthCalledWith(
      1,
      '/api/v1/dataset/list?order_by=-created_at',
      expect.objectContaining({
