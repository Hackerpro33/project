...
def _upgrade_entry(entry: Dict[str, Any]) -> Dict[str, Any]:
    """Return a backward-compatible view entry with normalized payloads."""

    upgraded = dict(entry)

    metrics = _normalize_metric_map(upgraded.get("metrics"))
    baseline_metrics = _normalize_metric_map(
        upgraded.get("baseline_metrics") or metrics
    )
    if not baseline_metrics:
        baseline_metrics = metrics

    delta = _normalize_metric_map(upgraded.get("delta"))
    if not delta:
        delta = compute_metrics_delta(metrics, _normalize_metric_map(upgraded.get("previous_metrics")))

    delta_from_baseline = _normalize_metric_map(upgraded.get("delta_from_baseline"))
    if not delta_from_baseline:
        delta_from_baseline = compute_metrics_delta(metrics, baseline_metrics)

    raw_history = upgraded.get("history", [])
    baseline_version_id = upgraded.get("baseline_version_id")
    if not baseline_version_id and raw_history:
        baseline_version_id = raw_history[0].get("version_id")
    ...
    refresh_count = int(upgraded.get("refresh_count", 0))
    if refresh_count <= 0:
        refresh_count = len(history) or (1 if metrics else 0)
    ...
    upgraded.update(
        {
            ...
            "row_count": int(upgraded.get("row_count", 0)),
            "strategy": upgraded.get("strategy", "incremental"),
            "last_refresh_at": int(upgraded.get("last_refresh_at") or last_event.get("refreshed_at", 0)),
            "last_refresh_date": upgraded.get("last_refresh_date") or last_event.get("refreshed_date", ""),
        }
    )

    upgraded.setdefault("baseline_version_id", first_event.get("version_id"))
    upgraded.setdefault("baseline_version_number", first_event.get("version_number"))
    upgraded.setdefault("last_version_id", last_event.get("version_id"))
    upgraded.setdefault("last_version_number", last_event.get("version_number"))
